import {
    View, Text, TouchableOpacity, StyleSheet,
    SafeAreaView, Animated,
} from 'react-native';
import { router } from 'expo-router';
import { useOnboarding } from '../../context/OnboardingContext';
import { useRef, useEffect } from 'react';
import { Ionicons } from '@expo/vector-icons';

type ActivityLevel = 'sedentary' | 'light' | 'moderate' | 'very_active' | 'athlete';

const LEVELS: { key: ActivityLevel; label: string; desc: string; icon: keyof typeof Ionicons.glyphMap }[] = [
    { key: 'sedentary',   label: 'Sedentary',    desc: 'Desk job, little to no exercise',       icon: 'laptop-outline' },
    { key: 'light',       label: 'Light',         desc: '1–3 light workouts per week',           icon: 'walk-outline' },
    { key: 'moderate',    label: 'Moderate',      desc: '3–5 moderate workouts per week',        icon: 'bicycle-outline' },
    { key: 'very_active', label: 'Very Active',   desc: '6–7 hard workouts per week',            icon: 'fitness-outline' },
    { key: 'athlete',     label: 'Athlete',       desc: 'Twice-daily training / physical job',   icon: 'trophy-outline' },
];

export default function ActivityScreen() {
    const { data, update } = useOnboarding();
    const fade = useRef(new Animated.Value(0)).current;
    const slide = useRef(new Animated.Value(24)).current;

    useEffect(() => {
        Animated.parallel([
            Animated.timing(fade,  { toValue: 1, duration: 400, useNativeDriver: true }),
            Animated.timing(slide, { toValue: 0, duration: 400, useNativeDriver: true }),
        ]).start();
    }, []);

    return (
        <SafeAreaView style={styles.safe}>
            {/* Progress */}
            <View style={styles.progressRow}>
                {[1,2,3,4,5,6,7].map(i => (
                    <View key={i} style={[styles.pip, i <= 3 && styles.pipActive]} />
                ))}
                <Text style={styles.stepLabel}>3 of 7</Text>
            </View>

            <Animated.View style={[styles.content, { opacity: fade, transform: [{ translateY: slide }] }]}>
                <Text style={styles.title}>How active are you?</Text>
                <Text style={styles.sub}>This sets your daily calorie multiplier.</Text>

                <View style={styles.list}>
                    {LEVELS.map(l => {
                        const active = data.activity_level === l.key;
                        return (
                            <TouchableOpacity
                                key={l.key}
                                style={[styles.card, active && styles.cardActive]}
                                onPress={() => update({ activity_level: l.key })}
                                activeOpacity={0.8}
                            >
                                <View style={[styles.iconWrap, active && styles.iconWrapActive]}>
                                    <Ionicons name={l.icon} size={22} color={active ? '#000' : '#00E676'} />
                                </View>
                                <View style={styles.cardText}>
                                    <Text style={[styles.cardTitle, active && styles.cardTitleActive]}>{l.label}</Text>
                                    <Text style={styles.cardDesc}>{l.desc}</Text>
                                </View>
                                {active && <Ionicons name="checkmark-circle" size={20} color="#00E676" />}
                            </TouchableOpacity>
                        );
                    })}
                </View>
            </Animated.View>

            <View style={styles.footer}>
                <TouchableOpacity style={styles.backBtn} onPress={() => router.back()}>
                    <Ionicons name="arrow-back" size={20} color="#888" />
                </TouchableOpacity>
                <TouchableOpacity
                    style={[styles.btn, !data.activity_level && styles.btnDisabled]}
                    disabled={!data.activity_level}
                    onPress={() => router.push('/(onboarding)/food-prefs')}
                >
                    <Text style={styles.btnText}>Continue</Text>
                    <Ionicons name="arrow-forward" size={18} color="#000" />
                </TouchableOpacity>
            </View>
        </SafeAreaView>
    );
}

const styles = StyleSheet.create({
    safe: { flex: 1, backgroundColor: '#0A0A0A' },
    progressRow: { flexDirection: 'row', alignItems: 'center', gap: 5, paddingHorizontal: 24, paddingTop: 16, paddingBottom: 8 },
    pip: { flex: 1, height: 4, borderRadius: 2, backgroundColor: 'rgba(255,255,255,0.12)' },
    pipActive: { backgroundColor: '#00E676' },
    stepLabel: { color: 'rgba(255,255,255,0.35)', fontSize: 11, fontWeight: '600', marginLeft: 4 },

    content: { flex: 1, paddingHorizontal: 24, paddingTop: 32 },
    title: { fontSize: 30, fontWeight: '800', color: '#FFF', marginBottom: 8, letterSpacing: -0.5 },
    sub: { fontSize: 15, color: '#666', marginBottom: 28 },

    list: { gap: 10 },
    card: {
        flexDirection: 'row', alignItems: 'center', gap: 14,
        backgroundColor: 'rgba(255,255,255,0.04)', borderRadius: 16, padding: 14,
        borderWidth: 1.5, borderColor: 'rgba(255,255,255,0.08)',
    },
    cardActive: { borderColor: '#00E676', backgroundColor: 'rgba(0,230,118,0.06)' },
    iconWrap: { width: 44, height: 44, borderRadius: 12, backgroundColor: 'rgba(0,230,118,0.10)', alignItems: 'center', justifyContent: 'center' },
    iconWrapActive: { backgroundColor: '#00E676' },
    cardText: { flex: 1 },
    cardTitle: { fontSize: 15, fontWeight: '700', color: '#FFF', marginBottom: 2 },
    cardTitleActive: { color: '#00E676' },
    cardDesc: { fontSize: 12, color: '#555' },

    footer: { flexDirection: 'row', gap: 12, paddingHorizontal: 24, paddingBottom: 32 },
    backBtn: {
        width: 56, height: 56, borderRadius: 16, borderWidth: 1.5,
        borderColor: 'rgba(255,255,255,0.12)', alignItems: 'center', justifyContent: 'center',
    },
    btn: {
        flex: 1, backgroundColor: '#00E676', borderRadius: 16, height: 56,
        flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8,
    },
    btnDisabled: { opacity: 0.35 },
    btnText: { fontSize: 16, fontWeight: '800', color: '#000' },
});
